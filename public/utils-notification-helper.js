self.orderStatusDescriptionMap = {
...
self.getNotificationType = function(notification) {
  ...
  if (title.includes("đơn dịch vụ đang xử lí !"))
    return "order_update_customer";
  if (title.includes("xử lí đơn")) return "order_update_manager";
  if (title.includes("đơn dịch vụ vừa được cập nhật"))
    return "order_update_staff";
 ...
 */
self.getFormattedNotificationContent = function(notification) {
  ...
  switch (type) {
    ...
    case "task_update": {
      ...
      const orderIdMatch = notification.content.match(/Mã đơn\s*:\s*(\w+)/);
    ...
    case "order_update_customer": {
      ...
      const orderId = orderIdMatch?.[1] || "";
      const statusText =
        orderStatusDescriptionMap[orderStatus] || "Trạng thái không xác định";
