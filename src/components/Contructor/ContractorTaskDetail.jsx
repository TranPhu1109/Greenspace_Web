...
const ContractorTaskDetail = () => {
  ...
  const getImages = () => {
    // Ưu tiên ảnh mẫu thiết kế nếu có
    if (order?.serviceType === "UsingDesignIdea" && designIdea?.image) {
      return [
    ...
    // Ngược lại là ảnh của đơn hàng thường
    if (order?.image) {
      return [
      ...
      if (taskResponse.status === 200) {
        ...
        if (taskResponse.data.serviceOrderId) {
          const orderResponse = await api.get(
            `/api/serviceorder/${taskResponse.data.serviceOrderId}`
          );
              ...
              const detailedProducts = await Promise.all(
                orderData.serviceOrderDetails.map(async (detail) => {
                  ...
                  return {
                    ...
                    productImage:
                      product && product.image ? product.image.imageUrl : null,
                  };
                })
              );
    ...
    try {
      ...
      if (task.serviceOrderId) {
        const orderResponse = await api.put(
          `/api/serviceorder/status/${task.serviceOrderId}`,
          {
            status: orderStatus,
            reportManger: "",
            reportAccoutant: "",
  ...
  const handleStartInstallation = () => {
    checkTimeAndStartInstallation(false);
  };

  const handleStartReinstallation = () => {
    checkTimeAndStartInstallation(true);
  };
  ...
  const handleCancelInstallation = () => {
    Modal.confirm({
      ...
      okText: "Xác nhận",
      cancelText: "Hủy",
      onOk: () => updateTaskStatus(11, 10), // 11 = Cancel for task, 10 = DeliveryFail for order
    });
  };
      ...
      <div className="back-navigation">
        <Button
          ...
          onClick={() => {
            ...
            else navigate("/contructor/tasks");
          }}
        >
          Quay lại
        </Button>
          ...
          <Row gutter={[16, 16]}>
              ...
              {order &&
                ...
                order.externalProducts.length > 0 && (
                  <Card
                    ...
                    className="external-products-card"
                  >
                    <Table
            ...
            <Col xs={24} lg={8}>
              ...
              <Card title="Hình ảnh đơn hàng" className="order-images-card">
                ...
                {/* Ảnh nhỏ bên dưới */}
                <Row gutter={12} justify="center" style={{ marginTop: 16 }}>
                  {images.map((img, idx) => (
                    <Col key={img} style={{ cursor: "pointer" }}>
                      <Image
                        src={img}
                        alt={`Ảnh ${idx + 1}`}
                        width={64}
          ...
          <Card className="description-card">
            ...
            {order?.serviceType === "UsingDesignIdea" && designIdea ? (
              ...
              <div
                className="html-content"
                dangerouslySetInnerHTML={{
                  __html: order?.description || "Không có mô tả",
                }}
              />
            )}
          </Card>
          ...
          <TabPane tab="Tài liệu hướng dẫn thiết kế" key="design-guides">
            <Card className="design-guides-card">
              <Row gutter={[24, 24]}>
                {/* Design Guide Images */}
                {designIdea.designImage1URL &&
                  !isPDF(designIdea.designImage1URL) &&
                  !isVideo(designIdea.designImage1URL) && (
                    <Col
                      xs={24}
                      md={24}
                      lg={
                        designIdea.designImage2URL || designIdea.designImage3URL
                          ? 12
                          : 24
                      }
                    >
                      <Card
                        title={
                          <>
                            <FileImageOutlined /> Hình ảnh hướng dẫn
                          </>
                        }
                        ...
                        extra={
                          <Button
                            type="link"
                            href={designIdea.designImage1URL}
                            target="_blank"
                          >
                            Mở trong tab mới
                          </Button>
                ...
                {/* PDF Documents */}
                {designIdea.designImage2URL &&
                  isPDF(designIdea.designImage2URL) && (
                    <Col
                      xs={24}
                      md={24}
                      lg={
                        designIdea.designImage1URL || designIdea.designImage3URL
                          ? 12
                          : 24
                      }
                    >
                      <Card
                        ...
                        extra={
                          <Button
                            type="link"
                            href={designIdea.designImage2URL}
                            target="_blank"
                          >
                            Mở trong tab mới
                          </Button>
                ...
                {/* Video Guides */}
                {designIdea.designImage3URL &&
                  isVideo(designIdea.designImage3URL) && (
                    <Col
                      xs={24}
                      md={24}
                      lg={
                        designIdea.designImage1URL || designIdea.designImage2URL
                          ? 12
                          : 24
                      }
                    >
                      <Card
                        ...
                        extra={
                          <Button
                            type="link"
                            href={designIdea.designImage3URL}
                            target="_blank"
                          >
                            Mở trong tab mới
                          </Button>
                        ...
                        dataSource={[
                          {
                            ...
                            title: isPDF(designIdea.designImage1URL)
                              ? "Tài liệu PDF"
                              : isVideo(designIdea.designImage1URL)
                              ? "Video hướng dẫn"
                              : "Hình ảnh hướng dẫn",
                            icon: isPDF(designIdea.designImage1URL) ? (
                              <FilePdfOutlined
                          ...
                          },
                          {
                            ...
                            title: isPDF(designIdea.designImage2URL)
                              ? "Tài liệu PDF"
                              : isVideo(designIdea.designImage2URL)
                              ? "Video hướng dẫn"
                              : "Hình ảnh hướng dẫn",
                            icon: isPDF(designIdea.designImage2URL) ? (
                              <FilePdfOutlined
                                style={{ fontSize: "24px", color: "#ff4d4f" }}
                              />
                            ) : isVideo(designIdea.designImage2URL) ? (
                          ...
                          },
                          {
                            ...
                            title: isPDF(designIdea.designImage3URL)
                              ? "Tài liệu PDF"
                              : isVideo(designIdea.designImage3URL)
                              ? "Video hướng dẫn"
                              : "Hình ảnh hướng dẫn",
                            icon: isPDF(designIdea.designImage3URL) ? (
                              <FilePdfOutlined
                                style={{ fontSize: "24px", color: "#ff4d4f" }}
                              />
                            ) : isVideo(designIdea.designImage3URL) ? (
                          ...
                          (item) =>
                            item.url &&
                            ((item.key === "designImage1URL" &&
                              (isPDF(item.url) || isVideo(item.url))) ||
                              (item.key === "designImage2URL" &&
                                !isPDF(item.url)) ||
                              (item.key === "designImage3URL" &&
                                !isVideo(item.url)))
                        )}
                        renderItem={(item) => (
                          <List.Item>
